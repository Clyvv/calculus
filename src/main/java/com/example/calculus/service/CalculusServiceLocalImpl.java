...
public class CalculusServiceLocalImpl implements CalculusService {
    ...
    private List<SimpleEntry<String, Object>> parse(int position, List<SimpleEntry<String, Object>> result,
                                                      String expression) throws Exception {

        if (position > expression.length()) {
            return result;
        }
        char expressionChar = expression.charAt(position);
        ...
        switch (expressionChar) {
            ...
            default:
                ...
                while (position < expression.length() && (Character.isDigit(expressionChar) || expressionChar == '.' || expressionChar == ',')) {
                    buildNumber.append(expressionChar);
					position++;
                    if (position < expression.length()) {
                        expressionChar = expression.charAt(position);
                    }
                }
        ...
        if (position == expression.length()) {
            ...
            return parse(position, result, expression);
        }
